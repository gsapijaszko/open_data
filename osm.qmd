...
Examples of country boundaries on different maps, source: [Archiwum Map Wojskowego Instytutu Geograficznego 1919-1939](http://english.mapywig.org)
:::
...
nat_parks |>
  ...
  subset(select = c("short_name", "operator", "related_law", "start_date", "wikipedia", "wikidata")) |>
  sf::st_drop_geometry() |>
  kableExtra::kable(row.names = FALSE) |>
  kableExtra::kable_classic()
```
...
As the layer contains `r nrow (l)` different features (including land usage, buildings, parks, forests etc.) and it takes `r format(as.numeric(object.size(l)/10^6), digits = 1)` Mb, it not necessary make sense to load it in whole. As we are interested in postal area boundaries, let utilize `query=` parameter.[^osm-5]

[^osm-5]: Other option would be load all fields including `other_tags` (which are like `hstore` type field in Postgre) and then massage it a bit with `tidyverse` functions:
...
## Land coverage and use {#land-coverage}
...
tmap::tm_shape(lu) +
  tmap::tm_polygons(fill = "type", 
                    fill.scale = tmap::tm_scale(values = c("orange", "darkgreen", "blue"))
  ) +
  tmap::tm_basemap()
```
...
::: column-margin
![](https://upload.wikimedia.org/wikipedia/commons/e/ee/PL_road_sign_C-13-16.svg){width="50px"}

![](https://upload.wikimedia.org/wikipedia/commons/2/2e/PL_road_sign_C-13-16a.svg){width="50px"}
:::
...
@fig-leipzig_plots visualizes road networks and the density of it.
...
This section was inspired by OpenData StackExchange [question](https://opendata.stackexchange.com/questions/19973/is-there-a-publicly-accessible-map-of-the-swedish-medium-voltage-electricity-gri/20658#20658).
...
sw <- osmdata::opq(bbox, timeout = 60*20) |>
  osmdata::add_osm_feature(key = "boundary", value = "administrative") |>
  osmdata::add_osm_feature(key = "admin_level", value = "2") |>
...
p <- osmdata::opq(bbox, timeout = 60*20) |>
  osmdata::add_osm_feature(key = "power") |>
  osmdata::osmdata_sf() |>
  osmdata::unique_osmdata()

grid <- p$osm_lines |>
  subset(power %in% c("line", "minor_line")) |>
