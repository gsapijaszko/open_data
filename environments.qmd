...
There are few packages which can be used to download the data: {KrigR} [@kuschKrigRToolDownloading2022] and {ecmwfr} [@ecmwfr; @ecmwfr2019]. In below an example, based on {ecmwfr}, we are going to check weather conditions in April and May of 1945 in Remagen, Germany.^[Thats an response to [Open Data](https://opendata.stackexchange.com/questions/20999/weather-conditions-for-remagen-germany-in-spring-of-1945) question] After creating ECMWF account and setting up the key in keyring using `wf_set_key()`^[for details please check package documentation] function we can create an request and download the data.
...
#| code-fold: true
#| fig-cap: Temperature in Remagen, Germany in April and May 1945
